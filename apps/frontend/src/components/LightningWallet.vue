          ...
          <div class="wallet-transactions-container" v-else>
            ...
            >
              <b-list-group-item
                ...
                :key="tx.hexPreImage || tx.paymentPreImage"
        ...
        >
          ...
          <div v-if="send.isValidInvoice && send.amount">
            <div class="d-flex justify-content-between mb-3 align-items-center">
              ...
              <small class="d-block text-muted"
                >~ {{ send.amount | satsToOtherUnit }}</small
              >
            </div>
        ...
        >
          ...
          <div class="mb-2">
            ...
            <small
              ...
              >~ {{ receive.amount | satsToOtherUnit }}</small
            >
          </div>
  ...
  methods: {
    ...
    async sendSats() {
      ...
      try {
        const res = await API.post(
          `${process.env.VUE_APP_API_BASE_URL}/v1/lnd/lightning/payInvoice`,
          payload
        );
    ...
    async createInvoice() {
      ...
      setTimeout(async () => {
        try {
          const res = await API.post(
            `${process.env.VUE_APP_API_BASE_URL}/v1/lnd/lightning/addInvoice`,
            payload
          );
