  ...
  if (confTarget === 0) {
    ...
    for (const key of keys) {
      if (baseFeeEstimate[key].feeSat) {
        baseFeeEstimate[key].feeSat = String(
          parseInt(baseFeeEstimate[key].feeSat, 10) +
            OPEN_CHANNEL_EXTRA_WEIGHT * baseFeeEstimate[key].feerateSatPerByte
        );
      }
    }
    ...
    baseFeeEstimate.feeSat = String(
      parseInt(baseFeeEstimate.feeSat, 10) +
        OPEN_CHANNEL_EXTRA_WEIGHT * baseFeeEstimate.feerateSatPerByte
    );
  }
...
async function estimateFeeSweep(
  address,
  fullAmtToEstimate,
  mempoolMinFee,
  confTarget,
  ...
  try {
    const successfulEstimate = await lndService.estimateFee(
      address,
      amtToEstimate,
      confTarget
    );
...
async function unlockWallet(password) {
  ...
  throw new LndError(
    "Lnd is not operational, therefore the wallet cannot be unlocked."
  );
}
...
function updateChannelPolicy(
  ...
  return lndService.updateChannelPolicy(
    global,
    fundingTxid,
    outputIndex,
    baseFeeMsat,
